...
### キャッシュ
ディスクの動作は遅いため、頻繁に使うデータは毎回ディスクから取り出したりせずに、  
「キャッシュ」と呼ばれるメモリ領域に格納しておき、すぐ使えるようにしておく。  

### ブロック
Oracleは「ブロック」という単位でデータを管理している。デフォルトは8KB。   
1つのブロックには複数の行が格納されているので、1行だけをディスクから読み込むためだとしても、  
欲しい行を含むブロックごとキャッシュに置かれる。

### 共有メモリ
原則、他のプロセスのメモリを見ることは出来ない。   
各プロセスが同じメモリ領域にアクセスすることが出来る「共有メモリ」という機能がある。  
Oracleでは、共有メモリをSGA(システムグローバルエリア)、  
共有でないサーバープロセスごとに存在するメモリをPGA(プログラムグローバルエリア)と言う。  
...
### SQL文の解析
SQL文には一般的なプログラミング言語と違って処理方法が書かれていない。  
オプティマイザがSQL文を解析し、「実行計画」という処理方法を作る。  
統計情報をもとに、処理時間やI/O回数が最小になると考えられる処理方法を算出する。  
基本的にはすべての処理方法のコストを計算して比較する。  

### 共有プール
実行計画を考えるためには、多量のCPUを使ってしまう。  
共有プールに実行計画をキャッシュして再利用し、リソース消費を抑える。  
ソフトパースは共有プールにキャッシュされている実行計画が存在し、再利用する場合の解析。  
ハードパースは共有プールに実行計画がないので、実行計画を作成する場合の解析。  
